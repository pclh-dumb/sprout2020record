...
struct BMP {
	...
	BMP(int width, int height){
		...
		fileHeader = FileHeader{{'B', 'M'}, 0, datasize+54, 54U};
	...
	bool read(string fn){
		...
		if(fileHeader.ID[0] != 'B' || fileHeader.ID[1] != 'M') {
	...
	bool save(string fn){
		...
		for(int i=0; i<fileHeader.data_offset - sizeof(fileHeader) - sizeof(bmpHeader); i++)
			fout.write("\0", 1);
